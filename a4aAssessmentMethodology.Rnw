...
<<res, fig.cap="Standardized residuals for survey (SNS, BTS-Tridens and BTS_Isis) and for catch numbers (catch.n), each panel is coded by age class (1:10), dots represent standardized residuals and lines is a simple fitted smoother.">>=
plot(res, main="Residuals")
@

The common bubble plot by year and age for each survey are shown in Figure~\ref{fig:bub}. 

<<bub, fig.cap="Bubbles plot of standardized residuals for survey (SNS, BTS-Tridens and BTS_Isis) and for catch numbers (catch.n).">>=
bubbles(res)
@
...
<<qq, fig.cap="Quantile-quantile plot of standardized residuals for survey (SNS, BTS-Tridens and BTS_Isis) and for catch numbers (catch.n), each panel is coded by age class (1:10), dots represent standardized residuals.">>=
qqmath(res)
@
...
population abundance (Figure~\ref{fig:N}), which is not estimated but that comes from the raw data and is thus displaid as a 3D ribbon, 
...
<<C, fig.cap="Catches at age by year">>=
...
persp3D( z = harvest(fit)[drop=TRUE], x= as.numeric(dimnames(harvest(fit))[[1]]), y= as.numeric(dimnames(harvest(fit))[[2]]) , expand = 0.9, space = 0.3, along = "xy",colkey = list( side=4, width = 0.9, length = 0.4, cex.axis = 0.8), box = TRUE, col.var=harvest(fit)[drop=TRUE] , zlab="Fishing mortality", ylab="Year", xlab="Age", theta=45, phi = 10)
@
...
persp3D( z = harvest(fit1)[drop=TRUE], x= as.numeric(dimnames(harvest(fit1))[[1]]), y= as.numeric(dimnames(harvest(fit1))[[2]]) , expand = 0.9, space = 0.3, along = "y",colkey = list( side=4, width = 0.9, length = 0.4, cex.axis = 0.8), box = TRUE, col.var=harvest(fit1)[drop=TRUE] , zlab="Fishing mortality", ylab="Year", xlab="Age", theta=45, phi = 10)
@
...
persp3D( z = harvest(fit3)[drop=TRUE], x= as.numeric(dimnames(harvest(fit3))[[1]]), y= as.numeric(dimnames(harvest(fit3))[[2]]) , expand = 0.9, space = 0.3, along = "y",colkey = list( side=4, width = 0.9, length = 0.4, cex.axis = 0.8), box = TRUE, col.var=harvest(fit3)[drop=TRUE] , zlab="Fishing mortality", ylab="Year", xlab="Age", theta=45, phi = 10)
@
...
persp3D( z = harvest(fit4)[drop=TRUE], x= as.numeric(dimnames(harvest(fit4))[[1]]), y= as.numeric(dimnames(harvest(fit4))[[2]]) , expand = 0.9, space = 0.3, along = "y",colkey = list( side=4, width = 0.9, length = 0.4, cex.axis = 0.8), box = TRUE, col.var=harvest(fit4)[drop=TRUE] , zlab="Fishing mortality", ylab="Year", xlab="Age", theta=55, phi = 10)
@
...
persp3D( z = (index(fit)[[1]]/stkn)[drop=TRUE], x= as.numeric(dimnames(index(fit)[[1]])[[1]]), y= as.numeric(dimnames(index(fit)[[1]])[[2]]) , expand = 0.9, space = 0.3, along = "y",colkey = list( side=4, width = 0.9, length = 0.4, cex.axis = 0.8), box = TRUE, col.var=harvest(fit)[drop=TRUE] , zlab="Catchability", ylab="Year", xlab="Age", theta=45, phi = 10, main="Index catchability")
@
...
persp3D( z = (index(fit1)[[1]]/stkn)[drop=TRUE], x= as.numeric(dimnames(index(fit1)[[1]])[[1]]), y= as.numeric(dimnames(index(fit1)[[1]])[[2]]) , expand = 0.9, space = 0.3, along = "y",colkey = list( side=4, width = 0.9, length = 0.4, cex.axis = 0.8), box = TRUE, col.var=harvest(fit1)[drop=TRUE] , zlab="Catchability", ylab="Year", xlab="Age", theta=45, phi = 10, main="Index catchability")
@
...
persp3D( z = (index(fit2)[[1]]/stkn)[drop=TRUE], x= as.numeric(dimnames(index(fit2)[[1]])[[1]]), y= as.numeric(dimnames(index(fit2)[[1]])[[2]]) , expand = 0.9, space = 0.3, along = "y",colkey = list( side=4, width = 0.9, length = 0.4, cex.axis = 0.8), box = TRUE, col.var=harvest(fit2)[drop=TRUE] , zlab="Catchability", ylab="Year", xlab="Age", theta=45, phi = 10, main="Index catchability")
...
persp3D( z = (index(fit3)[[1]]/stkn)[drop=TRUE], x= as.numeric(dimnames(index(fit3)[[1]])[[1]]), y= as.numeric(dimnames(index(fit3)[[1]])[[2]]) , expand = 0.9, space = 0.3, along = "x",colkey = list( side=4, width = 0.9, length = 0.4, cex.axis = 0.8), box = TRUE, col.var=harvest(fit3)[drop=TRUE] , zlab="Catchability", ylab="Year", xlab="Age", theta=45, phi = 10, main="Index catchability")
...
Residuals are not so good but one single biomass index is not very informative (Figure~\ref{fig:resbio}). Note that the results are not so bad because most information is coming from the catch at age information (Figure~\ref{fig:sbio}).

<<resbio, echo=FALSE, fig.cap="Catchability residuals for a biomass index">>=
plot(residuals(fit, ple4, FLIndices(bio=bioidx)))
@

<<sbio, echo=FALSE, fig.cap="Stock summary, rec = recruitment, harvest = fishing mortality, ssb = stock spawning biomass">>=
...
<<srmod, echo=FALSE, fig.cap="Recruitment models, fac = year as a factor, smo = smoother on year, ric = Ricker SR, bh = Beverton and Holt SR, hs = Hockey Stick SR, gm = Geometric mean SR">>=
...
<<ny1, echo=FALSE, fig.cap="Nay=1 models">>=
...
<<varmod, echo=FALSE, fig.cap="Population estimates using two different variance models">>=
...
persp3D( z = harvest(fit)[drop=TRUE], x= as.numeric(dimnames(harvest(fit))[[1]]), y= as.numeric(dimnames(harvest(fit))[[2]]) , expand = 0.9, space = 0.3, along = "xy",colkey = list( side=4, width = 0.9, length = 0.4, cex.axis = 0.8), box = TRUE, col.var=harvest(fit)[drop=TRUE] , zlab="Fishing mortality", ylab="Year", xlab="Age", theta=55, phi = 10)
@
...
\subsection{Mimicking other stock assessment models}
...
\subsubsection{VPA/XSA}
<<>>=
fmodel <- ~factor(replace(age, age>9, 9))+factor(year)
qmodel <- list(~factor(age), ~factor(age), ~factor(age))
fit <- sca(ple4, ple4.indices, fmodel=fmodel, qmodel=qmodel)
@

<<vpamod, echo=FALSE, fig.cap="VPA/XSA type of model">>=
persp3D( z = harvest(fit)[drop=TRUE], x= as.numeric(dimnames(harvest(fit))[[1]]), y= as.numeric(dimnames(harvest(fit))[[2]]) , expand = 0.9, space = 0.3, along = "xy",colkey = list( side=4, width = 0.9, length = 0.4, cex.axis = 0.8), box = TRUE, col.var=harvest(fit)[drop=TRUE] , zlab="Fishing mortality", ylab="Year", xlab="Age", theta=55, phi = 10)
@

\subsubsection{FLSAM}
<<>>=
fmod <- ~ factor(age) + s(year, k=10, by = breakpts(age, 1.5:10))
fit <- sca(ple4, ple4.indices, fmod)
@
