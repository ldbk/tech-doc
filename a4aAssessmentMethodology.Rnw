...
Summing other types of values such as weights and rates (like fishing mortality) does not make sense. Instead these values shoud be averaged over the length classes (weighted by the abundance for weights).
...
Within the a4a framework, the easiest way to insert natural mortality in the stock assessment is to use an \class{a4aM} object and run the method \code{m} to compute the values. The output is a \class{FLQuant} that can be directly inserted in the \class {FLStock} object to be used for the assessment.   

\subsection{\code{a4aM} - The M class}

Natural mortality is implemented in a class named \class{a4aM}. This class is made up of three models of the class \class{FLModelSim}. Each model represents one effect: an age effect, a year effect and a time trend, named \code{shape}, \code{level} and \code{trend}, respectively. Check the help files for more information.

<<showClass_a4aM>>=
showClass("a4aM")
@

The \class{a4aM} constructor requires that the models and parameters are provided. The default method will build each of these models as a constant value of 1. For example the usual "0.2" guessestimate could be set up by  
...
More interesting natural mortality shapes can be set up using biological knowledge. The following example uses Jensen's second estimator (Kenshington, 2013) $M=1.5K$ and an exponential decay to set up the level and shape of M.
...
%
Uncertainty on natural mortality is added through uncertainty on the parameters. In the case of \class{a4aM} class it makes use of the \class{FLModelSim} 'mvr' methods. A wrapper for \code{mvrnorm} was implemented, but all the other options must be carried out in each sub-model at the time.
...
level4 <- FLModelSim(model=~k^0.66*t^0.57, params=FLPar(matrix(c(0.4,10)), dimnames=list(params=c("k","t"), iter=1)), vcov=array(c(0.002, 0.01,0.01, 1), dim=c(2,2)))
...
The resulting parameter estimates can be seen in Figure~\ref{fig:plot_tri_gis_m}.

\begin{figure}
<<example_tri_gis_m_scatter>>=
...
Use the constructor or the set method to add the new model. Note that we have a quite complex method now for \emph{M}. A length based \code{shape} model from Gislason's work, Jensen's third based temperature \code{level} and a time \code{trend} depending on NAO. 
...
m(m1)
# With ages
rngage(m1) <- c(0,15) # set the age range
m(m1)
# with ages and years
...
The next example has an age based shape. The information on the range of ages can be passed when calling \code{m}, or else the method will pick it up from the \code{range} slot. Note that in this case \emph{mbar} becames relevant. It's the range of ages that is used to compute the mean level, which will match the \code{level} model.
% Eh? What is that stuff with matching level?
% And what is mbar?
...
See Figure~\ref{fig:uncertain_m}.
...
stks <- ple4 + fits
fit1 <- a4aSCA(stks, ple4.indices[1], fit="MP") 
@
